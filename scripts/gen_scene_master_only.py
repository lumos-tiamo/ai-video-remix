...
def run_scene(scene, image_dir, video_dir, port, log):
    ...
    if target_frames <= SAFE_MAX_FRAMES:
        log(f"scene{n}: single-shot generation, length={target_frames}")
        run_one(port, scene["video_prompt"], target_frames, f"remix/scene{n:02d}", 20000 + n,
                ref_image_name, final_path, log, f"scene{n}")
    else:
        ...
        run_one(port, scene["video_prompt"], len_a, f"remix/scene{n:02d}A", 20000 + n,
                ref_image_name, part_a, log, f"scene{n} partA")
        ...
        run_one(port, scene["video_prompt"], len_b, f"remix/scene{n:02d}B", 20100 + n,
                continuation_name, part_b, log, f"scene{n} partB")
...
if __name__ == "__main__":
    ...
    for sc in scenes:
        if sc.get("video_path"):
            ...
            continue
        try:
            run_scene(sc, image_dir, video_dir, port, log)
