---
name: ai-video-remix
description: 把一个源视频(二创/逆向还原)或一份原始素材/需求(原创写稿),端到端做成一条在 PalmierPro 里装配好的竖屏或横屏 AI 视频。覆盖文案+SRT提取、分镜提示词设计、首帧图生成、多场景 ComfyUI MiniMax-H3 集群视频生成、精确时间线装配。风格无关,已在 AI女团MV、噜噜二创(IP吉祥物)、掀牌说/老胡说说(时事财经锐评)、AI建筑、AI漫剧、AI女友 六种风格上验证过。当被要求"remix这个视频"、"把这段文案做成视频"、"照我们XX那套流程做个视频"等端到端AI视频生成需求时使用。
---
...
这是一套**风格无关的通用流水线**——阶段1-5的骨架(文案/歌词→分镜→首帧图→ComfyUI 集群视频生成→PalmierPro装配)对任何风格都成立,真正因风格而变的只是:画幅、画风描述、角色一致性用哪种机制、要不要配乐/配乐怎么来、场景之间要不要做像素级衔接。下面的"风格速查表"给出六种已经跑通过完整生产的风格的具体取值,拿到新需求先对号入座,而不是从零摸索。每一条写在这里的"坑"都是真的踩过才写的——跳过任何一步之前先读一遍,尤其是 GPU 集群和风格适配这两节。
...
六种风格的完整技术细节(具体脚本、参数、生产日志佐证)见 `references/style_playbooks.md`。**延时摄影(时间压缩镜头)目前只在 AI建筑 尝试过且没有做成功**——细节见下文阶段3专门一节,新需求如果要这个效果先看那一节,别重新踩一遍。
...
- **允许硬切镜头的题材**(时事解说、MV、IP吉祥物二创、漫剧、女友人设短剧——目前验证过的六种风格里,五种都是这样做的):不用真的做像素级衔接,靠**文字连续性**就够了——同一个位置/光线/穿搭描述在相邻场景的 `image_prompt`/`video_prompt` 里原样重复,运镜语言写成"承接上一镜的持续环绕运镜""echoing the opening shot"这类措辞,让观感上有连贯感,镜头之间该切就切。这条路便宜很多(不用等上一个场景先渲染完才能开始下一个,可以真并行),六种风格里默认应该选这个,只有明确要那种"真的没有剪辑点"的效果时才上前一条。
