---
name: ai-video-remix
description: 把一个源视频(二创/逆向还原)或一份原始素材/需求(原创写稿),端到端做成一条在 PalmierPro 里装配好的竖屏或横屏 AI 视频。覆盖文案+SRT提取、分镜提示词设计、首帧图生成、多场景 ComfyUI MiniMax-H3 集群视频生成、精确时间线装配。风格无关,已在 AI女团MV、噜噜二创(IP吉祥物)、掀牌说/老胡说说(时事财经锐评)、AI建筑、AI漫剧、AI女友、宠物喜剧二创(Cats&Dogs)、AI ASMR 等风格上验证过。当被要求"remix这个视频"、"把这段文案做成视频"、"照我们XX那套流程做个视频"等端到端AI视频生成需求时使用。
---
...
- **逆向还原素材本身是真实个人的第一人称/特写内容**(比如某个博主自己的手部特写视频,ASMR、手作、美甲这类内容常见)时,只复刻内容的类型和氛围(场景布置、动作、镜头语言),**不要复刻能指认到这个真实人物本人的具体身体特征**(手型、美甲款式、戒指/首饰这类)——写提示词时用"a person's hand"这种通用描述,不要照着源素材画面里具体的手描述细节,这跟改编真实IP角色是同一条原则在不同素材类型上的应用。
...
- **需要真实连续运镜感的题材**(比如"一镜到底"记录一次完整改造/建造过程,镜头始终不切黑幕、只是自然移动到下一个画面):每个场景生成完之后,用跟单场景内部拆分完全一样的手法——`ffmpeg -sseof -1` 精确抽出这个场景真实渲染出来的最后一帧,`POST /upload/image` 传上去,作为下一个场景的 `first_frame`/参考图,而不是重新用阶段2的静态首帧图。seed 建议递增而不是每个场景随机,视觉上更连贯。这样做出来的整条视频,画面之间的过渡是真实衔接的,不是靠碰运气对上的。**这条路径配合"同一场景多候选挑一个"(见上面阶段3)已经在一条完整8场景视频上端到端跑通**——每个场景都生成6-8个候选、挑一个最满意的(`chosen_port`/`chosen_seed` 记录在案),链式衔接到下一场景,最终装配出的成片总时长(60.47秒)跟目标时长(60.44秒的原片)精确对上,证明"多候选+链式首尾帧衔接"这个组合打法在全片规模上是可靠的,不只是单场景实验。
...
**后续这条视频最终还是完整交付了**(8个场景用"多候选挑一个"手法重新跑完、链式衔接、装配出成片,见上面阶段3"跨场景衔接"一节),但整个交付过程里的 `scenes.json` 从头到尾都没有出现任何速度验证字段或笔记——说明"延时摄影效果本身到底做出来了没有"这件事,即使在一条正式交付的成片里也没有被系统性验证过,只是换了个稳定出片的方法(多候选)把生成失败率降下去了,没有解决"生成出来的速度对不对"这个更本质的问题。**这进一步说明 (b) 条建议不是可有可无的**——现有流程里没有任何环节会自动帮你确认这件事,不主动做这一步,交付的成片大概率跟正常速度素材没有区别,只是没人核实过。
